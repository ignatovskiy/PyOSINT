...
class Parser:
    ...
    def get_text_from_ps(self, ps, clean_value=False, clean_key=False):
        ps_dict = dict()
        for p in ps:
            if p.i:
    ...
    def get_table_dict(self, parsed, headers=None):
        info_dict = dict()
        tables = self.get_all_elements_from_parent(parsed, 'table')
        if headers:
            tables_names = self.get_element_text(self.get_all_elements_from_parent(parsed, headers))
        else:
        ...
        for table, table_name in zip(tables, tables_names):
            ...
            rows_dict = self.parse_table(trs, parsed, collection_type='dict')
            if rows_dict:
                if table_name:
    ...
    def parse_table(self, trs, parsed, collection_type='list', first_row_index=0, headers=None, do_list=False, sep_text=True,
                    th_key=False):
        ...
        for tr in trs:
            tds_list = self.get_all_elements_from_parent(tr, 'td')
            tds_text_list = self.get_element_text(tds_list, sep_text=sep_text)
            if th_key:
                headers = self.get_element_text(self.get_all_elements_from_parent(tr, 'th'))
                if not headers:
                    continue
            row_dict_element = self.get_cells_data(first_row_index, headers, tds_text_list, do_list)
            if row_dict_element:
    ...
    def make_request(type_, url_, cookies_=None, data_=None, new_headers=None, use_default_headers=True):
        if use_default_headers:
            headers_ = {"User-Agent": constants.UA}
        else:
            headers_ = dict()
        if new_headers:
    ...
    def get_csrf_site_content(url, input_data):
        ...
        def get_posted_data(base_url, raw_data, csrf):
            headers = {'Referer': base_url, 'User-Agent': constants.UA}

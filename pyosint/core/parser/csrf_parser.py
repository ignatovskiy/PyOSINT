...
def get_posted_data(base_url: str, post_url: str, raw_data: dict, csrf: str):
    headers_: dict = {'Referer': base_url, 'User-Agent': UA}
    data: dict = {'csrfmiddlewaretoken': csrf}
    data.update(raw_data)
    cookies: dict = {'csrftoken': csrf}
    return requests.Session().post(post_url, cookies=cookies, data=data, headers=headers_).content
...
def get_csrf_site_content(url: str, post_url: str, input_data: dict) -> bSoup:
    ...
    return bSoup(get_posted_data(url, post_url, input_data, csrf_token), features="html.parser")